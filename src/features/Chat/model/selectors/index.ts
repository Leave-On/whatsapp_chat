...
export const getUserToken = (state: StateSchema) =>
	state?.chat?.apiTokenInstance;
export const getUserIsLoading = (state: StateSchema) =>
	state?.chat?.isLoading || false;
export const getChats = (state: StateSchema) => state.chat.chats || [];
export const getCurrentChatPhone = (state: StateSchema) =>
	state.chat.currentChatPhone || null;
